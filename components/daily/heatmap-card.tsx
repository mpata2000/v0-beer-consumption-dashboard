...
export function HeatmapCard({ data }: HeatmapCardProps) {
  ...
  const heatmapCounts = useMemo(() => {
    const matrix: number[][] = Array.from({ length: dayLabels.length }, () =>
      Array.from({ length: timeRanges.length }, () => 0)
    );
    for (const e of entries) {
      ...
      <CardContent>
        {timeRanges.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <div className="min-w-[280px] sm:min-w-[400px] md:min-w-[480px] lg:min-w-[640px]">
              <div
                className="grid gap-0.5 sm:gap-1"
                style={{
                  gridTemplateColumns: `60px repeat(${timeRanges.length}, minmax(40px, 1fr))`,
                }}
              >
                <div />
                {timeRanges.map((tr) => (
                ...
                {dayLabels.map((day, di) => (
                  <>
                    <div
                      key={`label-${day}`}
                      className="text-[10px] sm:text-xs text-muted-foreground py-1 flex items-center"
                    >
                      {day}
                    </div>
              ...
              <div className="flex items-center gap-2 mt-3 text-[10px] sm:text-xs text-muted-foreground">
                <span>Low</span>
                <div className="flex-1 h-1.5 sm:h-2 bg-gradient-to-r from-[rgba(59,130,246,0.2)] to-[rgba(59,130,246,0.8)] rounded" />
                <span>High</span>
              </div>
            </div>
          </div>
