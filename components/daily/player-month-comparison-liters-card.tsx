...
import { aggregateLitersPerMonthByPlayer, formatMonthKeyLabel } from "@/lib/data-utils";
...
function intensityColor(value: number, maxValue: number): string {
  if (!maxValue || value <= 0) return "rgba(16,185,129,0.08)"; // faint green
  const t = Math.min(1, value / maxValue);
  // green scale from light to strong
...
export function PlayerMonthComparisonLitersCard({ data }: PlayerMonthComparisonLitersCardProps) {
  ...
  // Get sorted player emails (by total liters descending)
  ...
  if (sortedPlayers.length === 0 || allMonths.length === 0) {
    return (
      <Card className="w-full">
        <CardHeader>
          <CardTitle>Comparación de Jugadores por Mes (Litros)</CardTitle>
          <CardDescription>
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        <CardTitle>Comparación de Jugadores por Mes (Litros)</CardTitle>
        <CardDescription>
          Cantidad de litros consumidos por jugador en cada mes
        </CardDescription>
      </CardHeader>
      <CardContent>
          ...
          >
            ...
            {allMonths.map((month) => (
              <>
                ...
                {sortedPlayers.map((email) => {
                  const monthMap = playerMonthData.get(email);
                  const liters = monthMap?.get(month) || 0;
                  return (
                    <div
                      key={`${email}-${month}`}
                      className="h-8 sm:h-10 rounded-sm flex items-center justify-center text-[10px] sm:text-xs font-medium"
                      style={{
                        backgroundColor: intensityColor(liters, maxValue),
                      }}
                      title={`${playerNames.get(email)} - ${formatMonthKeyLabel(month)}: ${liters.toFixed(1)}L`}
                    >
                      {liters > 0 ? liters.toFixed(1) : ""}
                    </div>
                  );
                })}
                <div
                ...
                >
                  {(monthlyTotals.get(month) || 0).toFixed(1)}
                </div>
              </>
            ))}
            ...
            {sortedPlayers.map((email) => (
              ...
              >
                {(playerTotals.get(email) || 0).toFixed(1)}
              </div>
            ))}
            <div className="h-8 sm:h-10 flex items-center justify-center text-[10px] sm:text-xs font-bold bg-primary/20 rounded-sm border-t-2 border-border">
              {Array.from(playerTotals.values()).reduce((sum, count) => sum + count, 0).toFixed(1)}
            </div>
          </div>
        </div>
      </CardContent>
